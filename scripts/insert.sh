#!/usr/bin/env bash
# Inserts a marker document into the current primary with majority write
# concern. Used live during the failover demo, so it must tolerate the
# instructor's chosen victim pod being unreachable: it execs into whichever
# member pod is currently reachable and lets the driver route the write to
# whichever member is primary via the full replica set seed list.
set -euo pipefail
